...
if h.wflag == 1
    ...
    if h.MATLABVER>=8.4
        if abs(PhOut)<1e-3
            set(h.TextOut,'String',['Output: $$\bf x(t)=' sprintf('%s',Astr) '\cos\left(' sprintf('%.1f',FoNout) '\,t\right)$$']);
        elseif abs(abs(PhOut)-pi)<1e-3
            set(h.TextOut,'String',['Output: $$\bf x(t)=' sprintf('%s',Astr) '\cos\left(' sprintf('%.1f',FoNout) '\,t + \pi\right)$$']);
        elseif PhOut<0
            set(h.TextOut,'String',['Output: $$\bf x(t)=' sprintf('%s',Astr) '\cos\left(' sprintf('%.1f',FoNout) '\,t - ' sprintf('%.2f',-PhOut) '\right)$$']);
        elseif PhOut>0
            set(h.TextOut,'String',['Output: $$\bf x(t)=' sprintf('%s',Astr) '\cos\left(' sprintf('%.1f',FoNout) '\,t + ' sprintf('%.2f',PhOut) '\right)$$']);
        end
    else
        if abs(PhOut)<1e-3
            set(h.TextOut,'String',sprintf('Output: x(t) = %scos(%.1f t)',Astr,FoNout),'Interpreter','tex');
        elseif abs(abs(PhOut)-pi)<1e-3
            set(h.TextOut,'String',sprintf('Output: x(t) = %scos(%.1f t + \\pi)',Astr,FoNout),'Interpreter','tex');
        elseif PhOut<0
            set(h.TextOut,'String',sprintf('Output: x(t) = %scos(%.1f t - %.2f)',Astr,FoNout,-PhOut),'Interpreter','tex');
        elseif PhOut>0
            set(h.TextOut,'String',sprintf('Output: x(t) = %scos(%.1f t + %.2f)',Astr,FoNout,PhOut),'Interpreter','tex');
        end
    end
    ...
    if h.MATLABVER>=8.4
        if abs(PhOut)<1e-3
            set(h.TextOut,'String',['Output: $$\bf x(t)=' sprintf('%s',Astr) '\cos\left(2\pi(' sprintf('%.1f',FoNout) ')\,t\right)$$'],'Interpreter','Latex');
        elseif abs(abs(PhOut)-pi)<1e-3
            set(h.TextOut,'String',['Output: $$\bf x(t)=' sprintf('%s',Astr) '\cos(2\pi\left(' sprintf('%.1f',FoNout) ')\,t + \pi\right)$$'],'Interpreter','Latex');
        elseif PhOut<0
            set(h.TextOut,'String',['Output: $$\bf x(t)=' sprintf('%s',Astr) '\cos(2\pi\left(' sprintf('%.1f',FoNout) ')\,t - ' sprintf('%.2f',-PhOut) '\right)$$'],'Interpreter','Latex');
        elseif PhOut>0
            set(h.TextOut,'String',['Output: $$\bf x(t)=' sprintf('%s',Astr) '\cos(2\pi\left(' sprintf('%.1f',FoNout) ')\,t + ' sprintf('%.2f',PhOut) '\right)$$'],'Interpreter','Latex');
        end       
    else
        if abs(PhOut)<1e-3
            set(h.TextOut,'String',sprintf('Output: x(t) = %scos(2\\pi %.1f t)',Astr,FoNout));
        elseif abs(abs(PhOut)-pi)<1e-3
            set(h.TextOut,'String',sprintf('Output: x(t) = %scos(2\\pi %.1f t + \\pi)',Astr,FoNout));
        elseif PhOut<0
            set(h.TextOut,'String',sprintf('Output: x(t) = %scos(2\\pi %.1f t - %.2f)',Astr,FoNout,-PhOut));
        elseif PhOut>0
            set(h.TextOut,'String',sprintf('Output: x(t) = %scos(2\\pi %.1f t + %.2f)',Astr,FoNout,PhOut));
        end
    end
